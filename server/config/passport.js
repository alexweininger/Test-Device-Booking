...
module.exports = function(passport) {
  ...
  passport.deserializeUser(function(id, done) {
    ...
    connection.query("SELECT * FROM atbl_Users WHERE id = ? ", [id], function(
      err,
      rows
    ) {
  ...
  passport.use(
    ...
    new LocalStrategy(
      ...
      function(
        ...
        connection.query(
          "SELECT * FROM atbl_Users WHERE username = ?",
          [username],
          function(err, rows) {
            ...
            if (rows.length) {
              return done(
                null,
                false,
                req.flash("That username is already taken.")
              );
            } else {
              var newUserMysql = {
                firstName: firstName,
              ...
              var insertQuery =
                "INSERT INTO atbl_Users ( firstName, lastName, email, slackUsername, id, officeId, role, password ) values (?,?,?,?,?,?,?,?)";
  ...
  passport.use(
    ...
    new LocalStrategy(
      ...
      function(req, email, password, done) {
        ...
        connection.query(
          "SELECT * FROM atbl_Users WHERE email = '" + email + "';",
          function(err, rows) {
