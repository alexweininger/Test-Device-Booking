...
router.post("/", function(req, res) {
	...
	if (err) {
		...
		const values = ` VALUES ('${user.firstName}', '${user.lastName}', '${user.email}', '${user.slackUsername}', '${user.id}', '${user.officeId}', '${
			user.role
		}');`;
	...
	const update = `
	UPDATE Users
	Set firstName='${user.firstName}', lastName='${user.lastName}', email='${user.lastName}', slackUsername='${user.slackUsername}', officeId='${
		user.officeId
	}, role='${user.role}'
	WHERE id='${user.id}';
	`;
