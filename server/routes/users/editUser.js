...
router.post("/", function(req, res) {
  ...
  if (err) {
    ...
    const update = `UPDATE atbl_Users Set firstName='${
      user.firstName
    }', lastName='${user.lastName}', email='${user.email}', slackUsername='${
      user.slackUsername
    }', officeId='${user.officeId}', role='${user.role}' WHERE id='${user.id}'`;
