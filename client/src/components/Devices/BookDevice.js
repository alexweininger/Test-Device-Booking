...
class BookDevice extends React.Component {
  ...
  render() {
    ...
    return (
      <div>
        <Button size="large" variant="contained" color="inherit" className={classes.button} onClick={this.handleClickOpen}>
          Book device
        </Button>
        <Dialog className={classes.dialog} open={this.state.open} onClose={this.handleClose} >
            <DialogTitle id="alert-dialog-title">{"Book device"}
                <DialogContent className={classes.dialog}>
                    ...
                    <Select  className={classes.input} color="inherit" value={this.state.selectedTimeValue} onChange={this.handleTimeChange} >
                            {time.map((t, index) =>
                                <MenuItem className={classes.input} key={index} value={index} primaryText={`$t`}/>
                                )}
                    </Select>
                        ...
                        <TableBody>
                            {rows.map(row => (
                                <TableRow className={classes.row} key={row.id}>
                                    ...
                                    <CustomTableCell align="left">{row.reservedBy}</CustomTableCell>
                                </TableRow>
                                ))}
                        </TableBody>
                    </Table>    
